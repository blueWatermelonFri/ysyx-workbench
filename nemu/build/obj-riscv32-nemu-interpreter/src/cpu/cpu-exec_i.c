...
#define macro(i) if ((i) >= len) goto finish; else { char c = str[i]; if (c != ' ') { Assert((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || c == '?', "invalid character '%c' in pattern string", c); __key = (__key << 4) | (c == '?' ? 0 : (c >= '0' && c <= '9') ? c - '0' : c - 'a' + 10); __mask = (__mask << 4) | (c == '?' ? 0 : 0xf); __shift = (c == '?' ? __shift + 4 : 0); } }
  ...
  do { if (!(0)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "pattern too long" "\33[0m" "\n")); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "/home/myuser/ysyx/ysyx-workbench/nemu/include/cpu/decode.h", 85, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
#undef macro
finish:
  *key = __key >> __shift;
  *mask = __mask >> __shift;
