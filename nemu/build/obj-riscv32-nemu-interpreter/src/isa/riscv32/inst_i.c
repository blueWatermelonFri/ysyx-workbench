...
static inline int check_reg_idx(int idx) {
  ((void) sizeof ((idx >= 0 && idx < 32) ? 1 : 0), __extension__ ({ if (idx >= 0 && idx < 32) ; else __assert_fail ("idx >= 0 && idx < MUXDEF(CONFIG_RVE, 16, 32)", "src/isa/riscv32/local-include/reg.h", 22, __extension__ __PRETTY_FUNCTION__); }));
  return idx;
}
