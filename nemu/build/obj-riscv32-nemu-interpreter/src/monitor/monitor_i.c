...
static void welcome() {
  do { printf("\33[1;34m" "[%s:%d %s] " "Trace: %s" "\33[0m" "\n", "src/monitor/monitor.c", 28, __func__, "\33[1;32m" "ON" "\33[0m"); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Trace: %s" "\33[0m" "\n", "src/monitor/monitor.c", 28, __func__, "\33[1;32m" "ON" "\33[0m"); fflush(log_fp); } } while (0); } while (0);
  do { printf("\33[1;34m" "[%s:%d %s] " "If trace is enabled, a log file will be generated " "to record the trace. This may lead to a large log file. " "If it is not necessary, you can disable it in menuconfig" "\33[0m" "\n", "src/monitor/monitor.c", 29, __func__); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "If trace is enabled, a log file will be generated " "to record the trace. This may lead to a large log file. " "If it is not necessary, you can disable it in menuconfig" "\33[0m" "\n", "src/monitor/monitor.c", 29, __func__); fflush(log_fp); } } while (0); } while (0);
  do { printf("\33[1;34m" "[%s:%d %s] " "Build time: %s, %s" "\33[0m" "\n", "src/monitor/monitor.c", 32, __func__, "22:45:22", "Oct 12 2024"); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Build time: %s, %s" "\33[0m" "\n", "src/monitor/monitor.c", 32, __func__, "22:45:22", "Oct 12 2024"); fflush(log_fp); } } while (0); } while (0);
...
static long load_img() {
  if (img_file == ((void *)0)) {
    do { printf("\33[1;34m" "[%s:%d %s] " "No image is given. Use the default build-in image." "\33[0m" "\n", "src/monitor/monitor.c", 50, __func__); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "No image is given. Use the default build-in image." "\33[0m" "\n", "src/monitor/monitor.c", 50, __func__); fflush(log_fp); } } while (0); } while (0);
    return 4096;
  }
  FILE *fp = fopen(img_file, "rb");
  do { if (!(fp)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "Can not open '%s'" "\33[0m" "\n", img_file)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((fp) ? 1 : 0), __extension__ ({ if (fp) ; else __assert_fail ("fp", "src/monitor/monitor.c", 55, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
  fseek(fp, 0, 2);
  long size = ftell(fp);
  do { printf("\33[1;34m" "[%s:%d %s] " "The image is %s, size = %ld" "\33[0m" "\n", "src/monitor/monitor.c", 60, __func__, img_file, size); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "The image is %s, size = %ld" "\33[0m" "\n", "src/monitor/monitor.c", 60, __func__, img_file, size); fflush(log_fp); } } while (0); } while (0);
  fseek(fp, 0, 0);
  int ret = fread(guest_to_host((((paddr_t)0x80000000) + 0x0)), size, 1, fp);
  ((void) sizeof ((ret == 1) ? 1 : 0), __extension__ ({ if (ret == 1) ; else __assert_fail ("ret == 1", "src/monitor/monitor.c", 64, __extension__ __PRETTY_FUNCTION__); }));
  fclose(fp);
  return size;
}
