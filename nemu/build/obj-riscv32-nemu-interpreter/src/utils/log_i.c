...
void init_log(const char *log_file) {
  ...
  if (log_file != ((void *)0)) {
    ...
    do { if (!(fp)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "Can not open '%s'" "\33[0m" "\n", log_file)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((fp) ? 1 : 0), __extension__ ({ if (fp) ; else __assert_fail ("fp", "src/utils/log.c", 27, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
    log_fp = fp;
  }
  do { printf("\33[1;34m" "[%s:%d %s] " "Log is written to %s" "\33[0m" "\n", "src/utils/log.c", 30, __func__, log_file ? log_file : "stdout"); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Log is written to %s" "\33[0m" "\n", "src/utils/log.c", 30, __func__, log_file ? log_file : "stdout"); fflush(log_fp); } } while (0); } while (0);
}
_Bool log_enable() {
