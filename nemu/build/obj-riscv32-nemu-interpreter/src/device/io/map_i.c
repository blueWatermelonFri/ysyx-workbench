...
uint8_t* new_space(int size) {
  ...
  p_space += size;
  ((void) sizeof ((p_space - io_space < (2 * 1024 * 1024)) ? 1 : 0), __extension__ ({ if (p_space - io_space < (2 * 1024 * 1024)) ; else __assert_fail ("p_space - io_space < IO_SPACE_MAX", "src/device/io/map.c", 31, __extension__ __PRETTY_FUNCTION__); }));
  return p;
}
