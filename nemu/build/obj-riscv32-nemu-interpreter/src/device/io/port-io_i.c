...
void add_pio_map(const char *name, ioaddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ...
  do { printf("\33[1;34m" "[%s:%d %s] " "Add port-io map '%s' at [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/device/io/port-io.c", 30, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Add port-io map '%s' at [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/device/io/port-io.c", 30, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); fflush(log_fp); } } while (0); } while (0);
  nr_map ++;
}
