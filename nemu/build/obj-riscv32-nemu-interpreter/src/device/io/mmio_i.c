...
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ...
  do { printf("\33[1;34m" "[%s:%d %s] " "Add mmio map '%s' at [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/device/io/mmio.c", 50, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Add mmio map '%s' at [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/device/io/mmio.c", 50, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); fflush(log_fp); } } while (0); } while (0);
  nr_map ++;
}
