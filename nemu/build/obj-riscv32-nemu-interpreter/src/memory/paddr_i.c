...
static inline word_t host_read(void *addr, int len) {
  switch (len) {
    ...
    ;
    default: ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "/home/myuser/ysyx/ysyx-workbench/nemu/include/memory/host.h", 27, __extension__ __PRETTY_FUNCTION__); }));
  }
}
static inline void host_write(void *addr, int len, word_t data) {
  switch (len) {
    ...
    ;
    default: ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "/home/myuser/ysyx/ysyx-workbench/nemu/include/memory/host.h", 37, __extension__ __PRETTY_FUNCTION__); }));
  }
}
...
static void out_of_bound(paddr_t addr) {
  do { if (!(0)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "address = " "0x%08" "x" " is out of bound of pmem [" "0x%08" "x" ", " "0x%08" "x" "] at pc = " "0x%08" "x" "\33[0m" "\n", addr, ((paddr_t)0x80000000), ((paddr_t)0x80000000 + 0x8000000 - 1), cpu.pc)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "src/memory/paddr.c", 40, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
}
void init_mem() {
  memset(pmem, rand(), 0x8000000);
  do { printf("\33[1;34m" "[%s:%d %s] " "physical memory area [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/memory/paddr.c", 50, __func__, ((paddr_t)0x80000000), ((paddr_t)0x80000000 + 0x8000000 - 1)); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "physical memory area [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/memory/paddr.c", 50, __func__, ((paddr_t)0x80000000), ((paddr_t)0x80000000 + 0x8000000 - 1)); fflush(log_fp); } } while (0); } while (0);
}
word_t paddr_read(paddr_t addr, int len) {
