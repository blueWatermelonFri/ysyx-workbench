...
// 根据csr指令的csr位域，即I型指令的imm，得到csr寄存器的索引
#define CSRIDX() do { *imm = USEXT(BITS(i, 31, 20), 12); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , SI, R(rd) = src1 >> imm);
  // srai是算数右移
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, uint64_t tmp = (uint64_t)(src1) * (uint64_t)(src2); R(rd) = (uint32_t)((tmp) >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, long tmp = (long) ((int)src1) * (uint64_t)(src2); ; R(rd) = (uint32_t)(((uint64_t) tmp) >> 32));
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret  , I, s->dnpc = isa_trap_return()); //
  // INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, s->dnpc = isa_raise_intr(11, s->pc)); //
  // INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, s->dnpc = isa_raise_intr(11, s->pc)); //
  // INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, s->dnpc = isa_raise_intr(11, s->pc)); //
  // INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, s->dnpc = isa_raise_intr(11, s->pc)); //
